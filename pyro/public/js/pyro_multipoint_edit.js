...
pyro.multipoint_edit = {
                ...
                frappe.call({
                    ...
                    callback: function (pf) {
                        ...
                        frappe.db.get_doc("Pyro Series Config", opts.template_name).then(templateDoc => {
                            ...
                            // Mandatory Sales Order child fields
                            ...
                            let pointColumns = (
                                includedPointFields.length
                                    ? pointFields.filter(pf2 =>
                                        includedPointFields.some(
                                            ip => ip.fieldname === pf2.fieldname
                                        )
                                    )
                                    : pointFields
                            ).map(c => ({
    ...
    _render: function (frm, itemColumns, pointColumns, allColumns, opts) {
        ...
        function loadItemRow(selection) {
            ...
            if (selection && selection !== "+ New Row") {
                ...
                currentItemRowUid =
                    itemData.custom_pyro_row_uid || null;
            } else {
                itemData = {};
        ...
        function mergeRow(pointData, itemData) {
            ...
            allColumns.forEach(c => {
                let val = c.is_item
                    ? (itemData[c.fieldname] || "")
                    : (pointData[c.fieldname] || "");
        ...
        function getRowsFromTable() {
            ...
            dialog.fields_dict.table_html.$wrapper
                .find("tbody tr")
                .each(function () {
            ...
            frappe.call({
                ...
                callback: function (r) {
                    ...
                    options.forEach(o => {
                        $datalist.append(`
                            <option
                                value="${frappe.utils.escape_html(o.value)}"
                            >
        ...
        dialog.fields_dict.row_picker.df.onchange =
            function () {
                loadItemRow(
                    dialog.get_value(
                        "row_picker"
    ...
    _save: function (
        ...
        if (
            ...
            frappe.msgprint(
                `Row 1 must have a valid ${anchor_field}.`
            );
        ...
        frappe.call({
            ...
            if (
                ...
                frappe.msgprint({
                    ...
                    message:
                        "None of the rows had a 'Point No' value filled in — nothing was saved. Fill the Point No column (e.g. U1, U2) for each row.",
            ...
            frappe.call({
                ...
                method:
                    "pyro.api.save_multipoint_points",
                ...
                callback:
                    function () {
                        ...
                        frappe.show_alert({
                            ...
                            message:
                                "Multipoint data saved successfully",
                ...
                error:
                    function (err) {
                        ...
                        frappe.msgprint({
                            ...
                            message:
                                "Item row was saved, but points failed to save. Check console (F12).",
    ...
    view_summary: function (
        ...
        if (
            ...
            frappe.msgprint(
                "No saved item rows found on this document yet."
            );
        ...
        if (
            ...
            pyro.multipoint_edit._show_summary(
                frm,
                opts,
                items[0]
            );
    ...
    _show_summary: function (
        ...
        let item_row_key =
            itemRow.custom_pyro_row_uid ||
            itemRow.name;

        frappe.call({

            method:
                "pyro.api.get_multipoint_summary",
            ...
            callback:
                function (r) {
                    ...
                    if (
                        ...
                        frappe.msgprint(
                            "No multipoint data saved for this item yet."
                        );
                    ...
                    let headerCells = `
                        ...
                        <th>TW Tag</th>
                        <th>Point No</th>
                        <th>Hot Junction Location mm</th>
                        <th>Head Extension N mm</th>
                        <th>Lagging Extension T mm</th>
                        <th>Overall Length OL mm</th>
                    `;
